...
auto visitASTNode(const Visitor& visitor, BaseASTNode& node,
  ...
  case ASTNodeType::ANCHOR: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeAnchor>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::ADD: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeAdd>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::ANY: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeAny>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::CHAR_CLASS: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeCharClass>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::CHAR_SEQUENCE: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeCharSequence>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::REPETITION: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeRepetition>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::OR: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeOr>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  case ASTNodeType::GROUP: {
    auto&& concreteNode = lightDowncast<FixConst<BaseASTNode, ASTNodeGroup>>(node);
    return applyVisitor(concreteNode, std::forward<Args>(args)...);
  }
  default: {
